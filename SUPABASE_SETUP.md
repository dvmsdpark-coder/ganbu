...
현재 SQL은 Supabase Auth와 RLS를 사용합니다. 승인된 사용자는 자기 부서 일정과 상위 조직에서 공유한 일정을 볼 수 있고, 총관리자는 사용자 승인과 권한 부여를 할 수 있습니다.

## 조직별 일정 공유 업데이트

기존에 데이터베이스를 만든 경우에도 Supabase SQL Editor에서 최신 `supabase-schema.sql` 전체를 다시 실행해야 합니다. 기존 일정은 유지되며 다음 항목이 추가됩니다.

- 장관실 일정의 선택적 전 부서 공개
- 차관실 일정의 선택적 하위 전 부서 공유
- 3개 실장실 일정의 선택적 소속 정책관·국 공유
- 공유 일정은 원본 부서만 수정·삭제하도록 제한
